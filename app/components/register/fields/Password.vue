...
const updateValue = (value: string) => (data.password = value)
</script>

<template>
  <label :class="registerErrors.password.isError ? 'input-error' : null">Password
    <input
      v-model="modelValue"
      name="password"
      required
  ...
  <span v-if="registerErrors.password.isError" class="form-error">{{ registerErrors.password.message }}</span>
</template>
