...
<template>
  <section v-if="store.currentCategory.totalPages" class="pagination pagination-top">
    ...
    <input :value="store.currentPage" type="number" class="pagination__page-number" :placeholder="`${store.currentPage}`" :max="store.currentCategory.totalPages" min="1" @input="pageFromInput($event as never, true as never)">
